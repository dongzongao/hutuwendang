...
| 三色标记算法 | 白灰黑三色含义、并发标记漏标问题、增量更新（CMS）与 SATB 原始快照（G1/ZGC）两种屏障方案对比 |
...
| 红黑树详解 | 五条性质、左旋右旋变色操作、插入删除平衡修复过程、与 AVL 树对比、在 TreeMap/HashMap/Linux CFS 中的应用 |
| MyBatis 核心原理 | JDK 动态代理生成 Mapper、MappedStatement 解析与缓存、SqlSource 动态 SQL 拼装、一级/二级缓存失效场景 |
| MySQL 锁详解 | 记录锁/间隙锁/临键锁加锁规则、意向锁兼容矩阵、MDL 元数据锁、死锁检测与排查 |
| 幻读详解 | 快照读与当前读的幻读场景、MVCC 解决快照读幻读、Next-Key Lock 解决当前读幻读、混用场景的边界问题 |
...
| 文档 | 深度内容 |
|------|---------|
| 锁概览 | 数据库锁、Java 并发锁、分布式锁全景图，各类锁分类与导航 |
| MySQL 锁详解 | 记录锁/间隙锁/临键锁加锁规则、意向锁兼容矩阵、MDL 元数据锁、死锁检测与排查 |
| 悲观锁与乐观锁 | FOR UPDATE 悲观锁、版本号乐观锁、CAS 原子操作、ABA 问题与重试风暴 |
| synchronized 详解 | Mark Word 对象头结构、偏向锁→轻量级锁→重量级锁升级过程、Monitor 监视器机制 |
| ReentrantLock 与 AQS | CLH 同步队列、state 状态机、公平/非公平锁实现、Condition 条件变量等待队列 |
| 读写锁详解 | ReadWriteLock 读写状态位设计、锁降级机制、StampedLock 乐观读三种模式 |
| Redis 分布式锁 | SETNX 原子加锁、Lua 脚本原子释放、Redisson 看门狗续期、RedLock 多节点算法 |
| ZooKeeper 分布式锁 | 临时顺序节点机制、监听前驱节点避免惊群效应、Curator 封装、与 Redis 锁对比 |
...
| MQ 兜底方案 | 生产者本地消息表与事务消息、Broker 同步刷盘与多副本、消费者手动 ACK 与死信队列、完整可靠性兜底体系 |
| MQ 架构设计 | 异步解耦、流量削峰、事件驱动、延迟消息、顺序消息、分布式事务六大场景方案，Kafka/RocketMQ/RabbitMQ 技术选型对比 |
...
| 算法题分类导航 | 哈希表、双指针、动态规划、栈队列、树与BFS、排序六大题型分类导航与解题框架 |
| 时间与空间复杂度 | 主定理（Master Theorem）、均摊分析、空间复杂度常见误区 |
| 哈希表算法题 | HashMap 一次遍历原理、字母异位词排序 key 分组、最长连续序列起点判断与 O(n) 扩展 |
| 双指针算法题 | 排序+双指针夹逼三数之和、左右最大高度预处理接雨水、滑动窗口字符位置跳跃去重 |
| 动态规划算法题 | 斐波那契滚动变量、Kadane 算法负前缀重置、完全背包状态转移方程与 dp 数组构建 |
| 栈与队列算法题 | 括号匹配状态机、双栈同步维护最小值、单调递减双端队列实现滑动窗口最大值 |
| 树与BFS算法题 | 队列逐层扩展层序遍历、递归后序求最大深度、DFS 路径剩余值叶子判断 |
| 排序算法题 | 快速排序 partition 分区原理、归并排序分治合并过程、各排序稳定性与复杂度对比 |
